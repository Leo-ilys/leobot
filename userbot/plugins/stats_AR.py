...
#                           STRINGS                           #
# =========================================================== #
STAT_INDICATION = "`جـاري جمع معـلوماتك ⌁`"
...
async def stats(event):
    ...
    response = f"⌁ **مـعلومات حـول 🔜 {full_name}** \n\n"
    response += f"**الـدردشات الخـاصة:** {private_chats} \n"
    response += f"   ⌁ `المـستخدمون: {private_chats - bots}` \n"
    response += f"   ⌁ `عـدد البـوتات: {bots}` \n"
    response += f"**المجـموعات:** {groups} \n"
    response += f"**القـنوات:** {broadcast_channels} \n"
    response += f"**الأدمنـية في المـجموعات:** {admin_in_groups} \n"
    response += f"   ⌁ `المنشـئ: {creator_in_groups}` \n"
    response += f"   ⌁ `حـقوق المسؤول: {admin_in_groups - creator_in_groups}` \n"
    response += f"**ادمـن في القنـوات:** {admin_in_broadcast_channels} \n"
    response += f"   ⌁ `المنشـئ: {creator_in_channels}` \n"
    response += (
        f"   ⌁ `حـقوق المسـؤول: {admin_in_broadcast_channels - creator_in_channels}` \n"
    )
    response += f"**الغيـر مقروئـه:** {unread} \n"
    response += f"**التـاكات الغـير مقـروئة:** {unread_mentions} \n\n"
    response += f"⌁ عدد الـدقائق : {stop_time:.02f}s \n"
