...
async def catbroadcast_send(event):
    ...
    if not catinput_str:
        return await edit_delete(
            event, "إلى أي فئة يجب أن أرسل هذه الرسالة ", parse_mode=parse_pre
        )
    reply = await event.get_reply_message()
    cat = base64.b64decode("QUFBQUFGRV9vWjVYVE5fUnVaaEtOdw==")
    if not reply:
        return await edit_delete(
            event, "ماذا علي أن أرسل إلى هذه الفئة؟ ", parse_mode=parse_pre
        )
    ...
    if no_of_chats == 0:
        return await edit_delete(
            event,
            f"لا توجد فئة بالاسم  {keyword}. للتـأكد أرسـل '.listall'",
            parse_mode=parse_pre,
        )
    chats = sql.get_chat_broadcastlist(keyword)
    catevent = await edit_or_reply(
        event,
        "إرسال هذه الرسالة إلى كافة المجموعات في الفئة ",
        parse_mode=parse_pre,
    )
    try:
    ...
    for chat in chats:
        try:
            ...
            LOGS.info(str(e))
        await sleep(0.5)
    resultext = f"`تم إرسال الرسالة إلى  {i} الدردشات من {no_of_chats} الدردشات في الفئة {keyword}.`"
    await catevent.edit(resultext)
    if BOTLOG:
        await event.client.send_message(
            BOTLOG_CHATID,
            f"تم إرسال رسالة إلى {i} الدردشات من {no_of_chats} الدردشات في الفئة {keyword} ",
            parse_mode=parse_pre,
        )
...
async def catbroadcast_send(event):
    ...
    if no_of_chats == 0:
        return await edit_delete(
            event,
            f"لا توجد فئة بالاسم {keyword}. تـاكد مـن خلال ارسـال أمر '.listall' ",
            parse_mode=parse_pre,
        )
    chats = sql.get_chat_broadcastlist(keyword)
    catevent = await edit_or_reply(
...
async def catbroadcast_add(event):
    ...
    if not catinput_str:
        return await edit_delete(
            event, "في أي فئة يجب أن أضيف هذه الدردشة ", parse_mode=parse_pre
        )
    keyword = catinput_str.lower()
    ...
    if check:
        return await edit_delete(
            event,
            f"هذه الدردشة موجودة بالفعل في هذه الفئة {keyword} ",
            parse_mode=parse_pre,
        )
    sql.add_to_broadcastlist(keyword, event.chat_id)
    await edit_delete(
        event, f"تمت إضافة هذه الدردشة الآن إلى الفئة {keyword} ", parse_mode=parse_pre
    )
...
async def catbroadcast_remove(event):
    ...
    if not catinput_str:
        return await edit_delete(
            event, "من أي فئة يجب أن أزيل هذه الدردشة ", parse_mode=parse_pre
        )
    keyword = catinput_str.lower()
    check = sql.is_in_broadcastlist(keyword, event.chat_id)
    if not check:
        return await edit_delete(
            event, f"هذه الدردشة ليست في الفئة : {keyword} ", parse_mode=parse_pre
        )
...
async def catbroadcast_list(event):
    ...
    if no_of_chats == 0:
        return await edit_delete(
            event,
            f"لا توجد فئة بالاسم : {keyword}. تأكد من خـلال ارسـال أمر  '.listall' ",
            parse_mode=parse_pre,
        )
    chats = sql.get_chat_broadcastlist(keyword)
    catevent = await edit_or_reply(
    ...
    resultlist = f"**الفـئة '{keyword}' لـديك '{no_of_chats}' الدردشات وهذه مذكورة أدناه  :**\n\n"
    errorlist = ""
    for chat in chats:
        try:
...
async def catbroadcast_list(event):
    ...
    resultext = "**فيما يلي قائمة الفئات الخاصة بك  :**\n\n"
    for i in chats:
...
async def catbroadcast_remove(event):
    ...
    if len(args) != 2:
        return await edit_delete(
            event,
            "استخدم بناء الجملة الصحيح كما هو موضح. frmfrom category_name groupid ",
            parse_mode=parse_pre,
        )
    try:
        groupid = int(args[0])
    ...
    await edit_delete(
        event,
        f"المحـادثة {groupid} تمت إزالته الآن من الفئة {keyword} ",
        parse_mode=parse_pre,
    )
    chat = await event.get_chat()
    if BOTLOG:
        try:
            await event.client.send_message(
                BOTLOG_CHATID,
                f"المـحادثة {chat.title} تمت إزالته من الفئة {keyword} ",
                parse_mode=parse_pre,
            )
        except Exception:
            await event.client.send_message(
