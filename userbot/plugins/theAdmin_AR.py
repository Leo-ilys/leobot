...
PP_TOO_SMOL = "**⌁ الصـورة صغيـرة جدًا**"
PP_ERROR = "**⌁ فشـل أثنـاء معالجـة الصـورة**"
NO_ADMIN = "**⌁ انـا لسـت مـديرآ فـي هـذا المجموعـة**"
NO_PERM = "**⌁ عـذرا ليـست لدي صـلاحيات كـافية**"
CHAT_PP_CHANGED = "**⌁ تـم تغير صورة المـجموعة**"
INVALID_MEDIA = "**⌁ الـعامل غير صـالح**"
...
async def set_group_photo(gpic):
    ...
    if not gpic.is_group:
        await edit_or_reply(gpic, "**⌁ انـا لسـت مـديرآ فـي هـذا المجموعـة**.")
        return
...
async def promote(promt):
    ...
    if not promt.is_group:
        await edit_or_reply(promt, "**⌁ لاأعـتقد أن هذا مجمـوعة**")
        return
    ...
    catevent = await edit_or_reply(promt, "**⌁ تم رفع مشرف بنجاح **")
    ...
    try:
        ...
        await catevent.edit("**تم الترقية بنجاح ⌁**")
...
async def demote(dmod):
    ...
    if not dmod.is_group:
        await edit_or_reply(dmod, "**⌁ لاأعـتقد أن هذا مجمـوعة**")
        return
    ...
    if not admin and not creator:
        ...
        return
    catevent = await edit_or_reply(dmod, "**⌁ تم حذف الاشراف بنجاح **")
    rank = "admeme"
    ...
    try:
        ...
        await catevent.edit(NO_PERM)
        return
    await catevent.edit("**⌁ تم حذف الاشراف بنجاح **")
    if BOTLOG:
...
async def ban(bon):
    if bon.fwd_from:
        return
    if not bon.is_group:
        await edit_or_reply(bon, "⌁ لاأعـتقد أن هذا مجمـوعة")
        return
...
async def nothanos(unbon):
    ...
    if not unbon.is_group:
        await edit_or_reply(unbon, "⌁ لاأعـتقد أن هذا مجمـوعة")
        return
    ...
    if not admin and not creator:
        ...
        return
    catevent = await edit_or_reply(unbon, "**⌁ جـاري فـك الحظر ..**")
    ...
    try:
        ...
        await catevent.edit(
            f"{_format.mentionuser(user.first_name ,user.id)} `⌁ تـم فـك الحـظر الان`"
        )
        if BOTLOG:
...
async def kick(usr):
    if usr.fwd_from:
        return
    if not usr.is_group:
        await edit_or_reply(usr, "⌁ لاأعـتقد أن هذا مجمـوعة")
        return
    ...
    if not user:
        return
    catevent = await edit_or_reply(usr, "⌁ جـاري الحـظر ...")
    try:
...
async def pin(msg):
    ...
    await edit_delete(msg, "**⌁ تـم التثـبيت بنـجاح**", 3)
...
async def pin(msg):
    ...
    if not to_unpin and options != "all":
        await edit_delete(msg, "**⌁ الرد على رسالة لإلغاء تثبيتها**", 5)
        return
    ...
    await edit_delete(msg, "**⌁ تـم فـك الـحظر بنـجاح**", 3)
    user = await get_user_from_id(msg.sender_id, msg)
    if BOTLOG and not msg.is_private:
        try:
            await msg.client.send_message(
...
async def _(event):
    ...
    if not event.is_group:
        await edit_or_reply(event, "**⌁ لاأعـتقد أن هذا مجمـوعة**")
        return
