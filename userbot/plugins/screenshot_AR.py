...
async def _(event):
    ...
    if Config.CHROME_BIN is None:
        ...
        return
    catevent = await edit_or_reply(event, "`جـاري الـتقاط سـكرين ... ⌁`")
    start = datetime.now()
    try:
        ...
        if not caturl:
            ...
            caturl = url(inputstr)
        if not caturl:
            await catevent.edit("`الـرابط غـير معـتمد عـذرا ⌁`")
            return
        driver.get(inputstr)
        await catevent.edit("`حسـاب القـياسات ⌁`")
        ...
        await catevent.edit("`تـم الايـقاف ⌁`")
        driver.close()
        message_id = None
