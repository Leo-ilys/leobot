...
async def cmd_list(event):
    ...
    if input_str:
        ...
        if HELPTYPE is True:
            help_string = f" قائمة الاوامـر الخـاصة بـسورس جـمثون لـ المسـتخدم : {ALIVE_NAME} \
...
async def info(event):
    ...
    input_str = event.pattern_match.group(1)
    if input_str == "text":
        string = "⌁ عدد {count} الاوامر الموجوده في {plugincount} السودو اضافات في سـورس جـمثون \n\n"
        catcount = 0
        plugincount = 0
        ...
        if len(string) > 4095:
            ...
            url = f"https://nekobin.com/{key}"
            reply_text = f"⌁ جميع اوامر  سـورس جـمثون  [تجدها هنا]({url})"
